...
- 本文件不覆盖 Web 前端渲染、静态资源托管或 Vite 开发代理；server-hosted Web 的根路径与静态资产行为应由 Rust server/API 测试覆盖
...
| 场景 | 预期 |
| --- | --- |
| `faus web --base-url <test-server>` | 打开测试 server 对应 Web URL，退出码为 0 |
| `FAUS_BASE_URL=<test-server> faus web` | 使用环境变量地址 |
| `FAUS_BASE_URL` 与 `--base-url` 同时存在 | 使用 `--base-url` |
| base URL 带尾随斜杠 | Web URL 使用规范化后的根 URL |
...
| 根路径返回 Web assets 未构建错误 | 退出码非 0，并报告可诊断失败，不改开 `/routes` |
