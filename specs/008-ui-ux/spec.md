...
- 来源可见（Sources Are Visible）：一旦来源内容进入库级结构化状态，应用就应提供库级来源清单视图，而不是把来源管理限制在来源根配置本身
...
- 应用壳层中的后台轮询、异步刷新或状态同步不得在当前详情对象未变化时周期性重建详情侧栏中的预览控件；当前选中对象的图片、视频或文档页预览不得因为无关状态刷新而重复加载
