...
- 当共享搜索控制项已经进入正式搜索契约时，搜索工作区应为其提供明确但轻量的入口；当前切片中，这至少包括正式过滤器草稿区与基于 `next_cursor` 的“继续加载更多结果”入口
...
- 当当前结果响应携带 `next_cursor` 时，搜索工作区必须允许用户在不离开当前页面、不清空已加载结果列表的前提下继续追加后续结果
