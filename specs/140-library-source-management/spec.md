...
- 来源管理只管理 Source 与 SourceRoot 的库内位置事实；跨路径、跨库的处理结果复用由 Source Content、ContentE2eIndexState、Asset、Unit 与 UnitIndex 承接
- 库级来源清单可以附带代表性 Asset 摘要与预览引用，用于支持来源浏览工作区中的预览优先详情；这类附加摘要仍从属于聚合来源观察面，不构成独立 source detail 工作流
...
- `FR-12` 当前阶段来源清单可以返回代表性 Asset 摘要与预览引用，以支撑来源浏览工作区的预览优先详情；缺少可预览对象时系统必须明确返回空摘要，而不是伪造预览。
