...
if [[ "$JSON" -eq 1 ]]; then
  PYTHONUNBUFFERED=1 "$GPU_ENV_PYTHON" "$ROOT_DIR/tools/python/smoke_text_search.py" --json
else
  echo "[info] Config: ${FAUNI_CONFIG_SOURCE#$ROOT_DIR/}"
  PYTHONUNBUFFERED=1 "$GPU_ENV_PYTHON" "$ROOT_DIR/tools/python/smoke_text_search.py"
fi
