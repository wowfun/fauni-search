...
5. 如需验证文档查询，再运行 `scripts/local/smoke-document-search.sh`
...
文档查询对应命令是 `scripts/local/smoke-document-search.sh --dev --json`。

快速无 GPU 检查入口：
- `scripts/local/check.sh`
- `pnpm --dir ui test:e2e`

`pnpm --dir ui test:e2e` 是当前最小 UI smoke。它固定使用 `--dev` 配置：若 `--dev` 服务已在运行则直接复用；若未运行则自行启动并在结束后只清理由自身启动的 `--dev` 服务。当前覆盖文本、图片、视频与文档四种查询模式的 happy path，以及 `not_ready`、无效导入拒绝、非图片/视频/PDF 查询上传拒绝、库内 `image` / `document_page` 直接作为 query image、库内 `video_segment` 直接作为 query video、以及 `document_page` 直接作为查询文档的路径。运行前仍需要先完成一次 `scripts/local/bootstrap-linux.sh --dev`。
...
- `scripts/local/smoke-video-search.sh`
  ...
  - 支持 `--json` 输出机器可读验证摘要
- `scripts/local/smoke-document-search.sh`
  - 在 app、sidecar 和 Qdrant 已启动后运行真实 GPU smoke
  - 验证查询文档上传、`/search/document`、整份 PDF 查询、页范围查询，以及库内 `document_page` 直接复用为查询文档的路径
  - 支持 `--json` 输出机器可读验证摘要
- `scripts/local/check.sh`
...
- `sidecar/`
  ...
  - 当前已接通 `query_embedding`、`image_query_embedding`、`video_query_embedding`、`document_embedding` 与 `document_query_embedding`
- `ui/`
  - 最小 Vite 工作台，当前已接通三栏工作流：左侧建库/导入/任务，中间统一 Text / Image / Video / Document 搜索与结果，右侧预览与详情
  ...
  - `Document` 模式当前支持 PDF 查询上传、可选 `start_page/end_page` 数字页范围、整份文档默认查询，以及把库内 `document_page` 结果对象直接作为查询文档
  - 临时上传查询图片会在运行期按过期窗口自动回收，过期后需重新上传
  - 临时上传查询文档、查询视频与查询图片都属于运行期资产；视频 smoke 默认允许从 local-only manifest 自动派生截图与 clip
